...
public class COSTestUtils extends org.junit.Assert {

  private static final Log LOG =
              LogFactory.getLog(COSTestUtils.class);

  /**
   * Read an object from COS
   *
   * @param fs filesystem
   ...
   *
   * If the test.fs.cos.name property is not set, this will
   ...
   */
  public static ObjectStoreFileSystem createTestFileSystem(Configuration conf)
      throws IOException {
  ...
  public static ObjectStoreFileSystem createTestFileSystem(Configuration conf,
    ...
    String fsname = conf.getTrimmed(COSTestConstants.TEST_FS_COS_NAME, "");

    boolean liveTest = !StringUtils.isEmpty(fsname);
    URI testURI = null;
    if (liveTest) {
      testURI = URI.create(fsname);
      liveTest = testURI.getScheme().equals(COSConstants.COS);
    }
    if (!liveTest) {
      // This doesn't work with our JUnit 3 style test cases, so instead we'll
      // make this whole class not run by default
      throw new AssumptionViolatedException(
          "No test filesystem in " + COSTestConstants.TEST_FS_COS_NAME);
    }
    ObjectStoreFileSystem fs1 = new ObjectStoreFileSystem();
  ...
  public static void disableFilesystemCaching(Configuration conf) {
    conf.setBoolean("fs.cos.impl.disable.cache", true);
  }
   ...
   * {@link COSTestConstants#TEST_UNIQUE_FORK_ID} if it is set.
   * @param defVal default value
   * @return a path
   */
  public static Path createTestPath(Path defVal) {
    String testUniqueForkId = System.getProperty(
        COSTestConstants.TEST_UNIQUE_FORK_ID);
